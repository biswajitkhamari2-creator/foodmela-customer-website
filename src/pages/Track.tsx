...
export default function Track() {
  ...
  useEffect(() => {
    ...
    // Primary: live Firestore listener (use normalized FM- ID matching backend writes)
    const unsub = onSnapshot(
      doc(db, 'orders', cleanOrderId),
      (snap) => {
        if (!alive) return;
        if (snap.exists()) {
          ...
          setErr('Order not found. Redirecting…');
    ...
    // Fallback: backend Redis poll (api.orderStatus normalizes internally)
    const poll = async () => {
      try {
        const res = await api.orderStatus(orderId);
        if (alive && res.order) {
          setOrder((res.order as LiveOrder));
          setErr('');
        }
      } catch {
